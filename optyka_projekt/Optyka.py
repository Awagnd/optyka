...
font = pygame.font.Font('SuperFunkyFreeFont.ttf', 200)
...
Światło_src = pygame.transform.rotate(Światło_src, 0)
Światło_src = pygame.transform.scale(Światło_src, (120, 75))
Swiatłosrc = Światło_src.get_rect()
Swiatłosrc.x = 100
Swiatłosrc.y = 80

WL_img = pygame.image.load("laser.png")
WL_img = pygame.transform.rotate(WL_img, 0)
WL_img = pygame.transform.scale(WL_img, (120,75))
WL = WL_img.get_rect()
...
pryzmat_img = pygame.transform.scale(pryzmat_img, (180, 120))
...
acc_img = pygame.image.load("add.png")
...
PointSlider5 = PointSlider_img.get_rect()
PointSlider5.x = Slider5.x+Slider5.width- PointSlider5.width / 2
PointSlider5.y = Slider5.center[1] - PointSlider5.height / 2
...
while running:
    ...
    if start == False:
        ...
        if b == 0:
            Part.append(Particle(random.randint(100, 1100), random.randint(200, 600), 3, (200, 200, 200),
                                 random.randint(0, 360), random.randint(1, 2)))
        a = 0
        for n in Part:
            if n.parpop:
                Part.pop(a)
        ...
        if Change:
            if Typ == 4:
                ...
                if properties==False:
                    ...
                    screen.blit(Slider_img, Slider5)
                    screen.blit(acc_img, acc)
                    screen.blit(PointSlider_img, PointSlider5)
                ...
                if substance_choice.height == 50:
                    if wc == 1:
                        ...
                        font_render("Kitchen salt", screen, substance_choice)
                else:
                    fonts_render(screen, 50, 100, ("glass quartz", "Flourite", "Kitchen salt"), substance_choice)
        ...
        for BALLS in YourJawz_img:
            ...
            if cpl[a] == (0, 0, 0):
                ...
                pygame.draw.polygon(screen, cpl[a], draw, 5)
            a += 1
                ...
                if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                    if Clicked1 == False:
                        if Swiatłosrc.collidepoint(mousePos):
                            ...
                            cp = (255, 255, 255)
                            pa=200
                            pryzmatypkt = [(110, 0), (110 - pa * 3 ** 0.5 / 3, pa), (110 + pa * 3 ** 0.5 / 3, pa)]
                            wc = 1
                            break
                    ...
                    if event.type == pygame.MOUSEBUTTONUP and event.button == 3:
                        if Typ == 1:
                            if trash.collidepoint(mousePos):
                                for n in range(20):
                                    Part.append(Particle(trash.center[0], trash.center[1], 7.5, (200, 200, 200),
                                                         random.randint(0, 360), random.randint(500, 1000) / 300))
                                SRCes.pop(leng)
                                KątŚ.pop(leng)
                                Light_quantity.pop(leng)
                        elif Typ == 2:
                            if trash.collidepoint(mousePos):
                                for n in range(20):
                                    Part.append(Particle(trash.center[0], trash.center[1], 7.5, (200, 200, 200),
                ...
                if Typ == 2:
                    if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                        if ZW.collidepoint(mousePos):
                            Raz = False
                            ZWierciadlas_img.append(pygame.transform.scale(pygame.image.load("fafa.png"),
                                                                           (10, 10 * lenHitboxZWierciadlas[leng])))
                    ...
                    if pygame.mouse.get_pressed()[0] == True:
                        if PointSlider.collidepoint(mousePos):
                            if Slider.x + Slider.width > mousePos[0] > Slider.x:
                                ...
                                lenHitboxZWierciadlas[leng] = int(
                                    10 + (PointSlider.x - Slider.x - Slider.width / 2) // 10)
                                print(lenHitboxZWierciadlas[leng])
                            Zwierciadło_img = pygame.transform.scale(Zwierciadło_img,
                                                                     (10, 10 * lenHitboxZWierciadlas[leng]))
                            ZW = Zwierciadło_img.get_rect()
                            ZW.x = 50
                    ...
                    if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                        if Swiatłosrc.collidepoint(mousePos):
                            ...
                            SRCes_img.append(pygame.transform.scale(pygame.image.load("laser.png"), (120, 80)))
                    ...
                    if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                        if properties==False:
                            if SocziRekt.collidepoint(mousePos):
                                ...
                                rei = (PointSlider3.center[0] - Slider3.x) / Slider3.width
                                if cp == (255, 255, 255):
                                    cp = (0, 0, 0)
                                cpl.append(cp)
                                # sys.exit()
                                ...
                                for ni in range(len(lens_hitboxes[-1])):
                                    CollHashmap[int(lens_hitboxes[-1][ni][0])][int(lens_hitboxes[-1][ni][1])] = (
                                    3, SoczewkiKąty[leng][ni])
                                break
                    if pygame.mouse.get_pressed()[0] == True:
                        if properties==False:
                            ...
                            if up2.collidepoint(mousePos):
                                ...
                                if Soczewkastrona == 1 and Soczewka2strona == 1 or SoczewkaX1[1] - 20 >= SoczewkaX2[1] and \
                                        SoczewkaX1[0] - 20 >= SoczewkaX2[0]:
                                    if Slider.x + Slider.width > mousePos[0] > Slider.x:
                                        PointSlider.x = mousePos[0] - PointSlider.width / 2
                    ...
                    if pygame.mouse.get_pressed()[0] == True:
                        if properties == False:
                            if PointSlider5.collidepoint(mousePos):
                                if Slider5.x + Slider5.width >= mousePos[0] >= Slider5.x:
                                    PointSlider5.x = mousePos[0] - PointSlider5.width / 2
                                    pa = (PointSlider5.center[0] - Slider5.x) / Slider5.width * 200
                                    pryzmatypkt = [(110, 0), (110 - pa * 3 ** 0.5 / 3, pa),
                                                   (110 + pa * 3 ** 0.5 / 3, pa)]
                        if PointSlider2.collidepoint(mousePos):
                            if Slider2.x + Slider2.width >= mousePos[0] >= Slider2.x:
                                PointSlider2.x = mousePos[0] - PointSlider2.width / 2
                            ...
                            if zakres.x + zakres.width > mousePos[0] > zakres.x:
                                ...
                                cp = screen.get_at((pointer.center[0], zakres.center[1]))
                                if properties:
                                    PrismCocklor[leng]=cp
                        elif substance_choice.collidepoint(mousePos):
                            if mousePos[1] < 125:
                                wc = 1
                                if properties:
                                    prism_material[leng]=wc
                            elif mousePos[1] < 175:
                                wc = 2
                                if properties:
                                    prism_material[leng]=wc
                    ...
                    if pygame.mouse.get_pressed()[0] == True:
                        ...
                        if pointer.collidepoint(mousePos):
                            ...
                            WLqua += 1
        if properties:
            if pygame.mouse.get_pressed()[0] and mousePos[1]>200:
                properties=False
                Change=False
                Raz=False
        if JDO:
            if SoczewkaR1 >= 50:
                if flat == False:
                    ...
                    for n in range(i + 1):
                        ...
                        if n == int(i / 2):
                            SoczewkaX1.append(SoczewkiRect[n][0]+20)
        ...
        if WeCooling and grubosc > 0:
            pygame.draw.rect(screen, (255, 0, 0), Rect(gruboschit[0][0], gruboschit[0][1],
                                                       gruboschit[int(len(gruboschit) / 2)][0] - gruboschit[0][0], 30))
        a = 0
        Soczewkamask = []
        for n in ZWierciadlas:
        ...
        print(PrismReflectiveindex)
        if pygame.mouse.get_pressed()[2] and Change==False and Clicked1==False:
            a=0
            for n in SocziRekts:
                if n.collidepoint(mousePos):
                    Typ=3
        ...
        if Clicked1:
            if Typ == 1:
                ...
                for ni in range(len(HitboxZWierciadlas[leng])):
                    if 1200 > int(HitboxZWierciadlas[leng][ni][0]) > 0 and 800 > int(
                            HitboxZWierciadlas[leng][ni][1]) > 0:
                ...
                for ni in range(len(HitboxZWierciadlas[leng])):
                    if 1200 > int(HitboxZWierciadlas[leng][ni][0]) > 0 and 800 > int(
                            HitboxZWierciadlas[leng][ni][1]) > 0:
                        CollHashmap[int(HitboxZWierciadlas[leng][ni][0])][int(HitboxZWierciadlas[leng][ni][1])] = (
                        2, ZWkąt[leng])
            elif Typ == 3:
                ...
                for n in range(len(YourJawz_img[leng])):
                    YourJawz_img[leng][n] = (
                    SocziRekts[leng].center[0] + DISt2[n][0], SocziRekts[leng].center[1] + DISt2[n][1])
                for ni in range(len(lens_hitboxes[leng])):
                    if 1200 > int(lens_hitboxes[leng][ni][0]) > 0 and 800 > int(lens_hitboxes[leng][ni][1]) > 0:
                        CollHashmap[int(lens_hitboxes[leng][ni][0])][int(lens_hitboxes[leng][ni][1])] = (
                        3, SoczewkiKąty[leng][ni],leng)
            elif Typ == 4:
                for n in range(3):
                ...
                for i in range(3):
                    ...
                    for n in range(len(pryzmatyhitbox[leng][i])):
                        pryzmatyhitbox[leng][i][n] = (
                        pryzmaty[leng].center[0] + DISt[i][n][0], pryzmaty[leng].center[1] + DISt[i][n][1])
                for n in range(3):
                    pryzmatyimg[leng][n] = (
                    pryzmaty[leng].center[0] + DISt2[n][0], pryzmaty[leng].center[1] + DISt2[n][1])
                for n in range(3):
                    for ni in range(len(pryzmatyhitbox[leng][n])):
                        if 1200 > pryzmatyhitbox[leng][n][ni][0] > 0 and 800 > pryzmatyhitbox[leng][n][ni][1] > 0:
                            CollHashmap[pryzmatyhitbox[leng][n][ni][0]][pryzmatyhitbox[leng][n][ni][1]] = (
                            4, pryzmatkaty[leng][n],leng)
            elif Typ == 5:
                ...
                while R[0][0] < 1200 and R[0][0] > 0 and R[0][1] > 200 and R[0][1] < 800:
                    ...
                    for x in SocziRekts:
                        if x.collidepoint(R[0]):
                            if Collck[0][0] == 3 and Collck[0][2]==b:
                                if lens_material[b] == 1:
                                    ir = glass_quartz_mm(700)
                                elif lens_material[b] == 2:
                                    ir = fluoryt_mm(700)
                                elif lens_material[b] == 3:
                                    ir = kitchensalt_mm(700)
                                if check[0] == 1:
                    ...
                    for x in pryzmaty:
                        if x.collidepoint(R[0]):
                            if Collck[0][0] == 4 and Collck[0][2]==b:
                                if prism_material[b] == 1:
                                    ir = glass_quartz_mm(700)
                                elif prism_material[b] == 2:
                                    ir = fluoryt_mm(700)
                                elif prism_material[b] == 3:
                                    ir = kitchensalt_mm(700)
                                if check2[0] == 1:
            ...
            for k in Wkolory[a]:
                ...
                while R[0][0] < 1200 and R[0][0] > 0 and R[0][1] > 200 and R[0][1] < 800:
                    ...
                    if Collck[0][0] == 2:
                        ...
                        Old = New
                    b = 0
                    print(lensReflectiveindex,lens_transparency)
                    for x in SocziRekts:
                        if x.collidepoint(R[0]):
                            if Collck[0][0] == 3 and Collck[0][2]==b:
                                if lens_material[b] == 1:
                                    ...
                                    ir = kitchensalt_mm(k[0])
                                if alfa * lensReflectiveindex[b] > 0.05:
                                    Reflections.append((Collck[1], k, Mcolors, alfa * lensReflectiveindex[b],
                                                        180 + 2 * (Collck[0][1] - 90) - kont, 0))
                                ...
                                if check[0] == 1:
                                    ...
                                    IRS = (1 + (ir - 1) * (lri[b] * 2 - 2))
                                    alfa *= lens_transparency[b]*(1-lensReflectiveindex[b])
                                    osidgh = 0
                        ...
                        if x.collidepoint(R[0]):
                            if Collck[0][0] == 4 and Collck[0][2]==b:
                                if prism_material[b] == 1 :
                                    ...
                                    ir = kitchensalt_mm(k[0])
                                if alfa * PrismReflectiveindex[b] > 0.05:
                                    Reflections.append((Collck[1], k, Mcolors, alfa * PrismReflectiveindex[b],
                                                        180 + 2 * (Collck[0][1] - 90) - kont, 0))
                        ...
                        if x.collidepoint(R[0]):
                            if Collck[0][0] == 3 and Collck[0][2]==b:
                                if lens_material[b] == 1:
                                    ...
                                    ir = kitchensalt_mm(k[0])
                                if alfa * lensReflectiveindex[b] > 0.1:
                                    Reflections.append((Collck[1], k, Mcolors, alfa * lensReflectiveindex[b],
                                                        180 + 2 * (Collck[0][1] - 90) - kont, 0))
                                New = R[0]
                                autistic_lines(screen, Old, New, White_width[a], k[1], alfa, Mcolors)
                        ...
                        if x.collidepoint(R[0]):
                            if Collck[0][0] == 4 and Collck[0][2]==b:
                                if prism_material[b] == 1:
                                    ...
                                    ir = kitchensalt_mm(k[0])
                                if alfa * PrismReflectiveindex[b] > 0.1:
                                    Reflections.append((Collck[1], k, Mcolors, alfa * PrismReflectiveindex[b],
                                                        180 + 2 * (Collck[0][1] - 90) - kont, 0))
                                New = R[0]
                                autistic_lines(screen, Old, New, White_width[a], k[1], alfa, Mcolors)
